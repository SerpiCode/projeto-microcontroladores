...
msg_header_modo3_dez_min:
    .db "[MODO 3] Ajustando a dezena dos minutos", 0

msg_header_modo3_uni_min:
    .db "[MODO 3] Ajustando a unidade dos minutos", 0

msg_header_modo3_dez_seg:
    .db "[MODO 3] Ajustando a dezena dos segundos", 0

msg_header_modo3_uni_seg:
    .db "[MODO 3] Ajustando a unidade dos segundos", 0
...
main_lp:
    ...
    brne no_blink_toggle
      ldi piscar_count, 0
	  com piscar_flag   ; Alterna piscar_flag
no_blink_toggle:
    ; so atualiza relogio/cronometro nos modos 0 e 1
    cpi  display_mode, 2
...
skip_timer:
    ...
    ; 4) Se for modo 3 (ajuste), chama ajustar_horario (le PC5/PC3 sempre)
    cpi  display_mode, 2
    breq ajustar_horario

    ; 5) Nos modos 0/1, trata start/reset (cron?metro)
...
;-----------------------------
send_char:
    ; Espera enquanto o registrador de dados n?o estiver vazio
Wait_UDRE:
    lds  r16, UCSR0A
    sbrs r16, UDRE0	; Checa se o registrador de dados est? vazio (livre para enviar)
    rjmp Wait_UDRE
    sts  UDR0, r17	; Grava o valor no endere?o de mem?ria UDR0 (registrador de dados UART)
    ret
...
send_modo2_serial:
	; Verifica se o cron?metro est? contando
    cpi cron_status, 1
    brne skip_modo2_message   ; Se n?o estiver contando, pula toda a fun??o
